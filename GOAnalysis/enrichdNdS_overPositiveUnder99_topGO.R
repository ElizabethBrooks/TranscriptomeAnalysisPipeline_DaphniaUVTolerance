...
get_interesting_DE_genes <- function(geneUniverse){
  ...
  for(i in 1:length(geneUniverse)){
    if(geneUniverse[i] > 6 & geneUniverse[i] < 99){
...
pdf(file="overPositiveUnder99_pValueRanges.pdf")
...
write.table(BP_GO_results_table, file="overPositiveUnder99_BP_GO_terms.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(MF_GO_results_table, file="overPositiveUnder99_MF_GO_terms.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(CC_GO_results_table, file="overPositiveUnder99_CC_GO_terms.csv", sep=",", row.names=FALSE, quote=FALSE)
...
write.table(BP_sigGO_results_table, file="overPositiveUnder99_BP_sigGO_terms.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(MF_sigGO_results_table, file="overPositiveUnder99_MF_sigGO_terms.csv", sep=",", row.names=FALSE, quote=FALSE)
write.table(CC_sigGO_results_table, file="overPositiveUnder99_CC_sigGO_terms.csv", sep=",", row.names=FALSE, quote=FALSE)
...
pdf(file = "overPositiveUnder99_TopSigGO_Density.pdf")
...
printGraph(BP_GO_data, BP_GO_results, firstSigNodes = 5, 
           fn.prefix = "overPositiveUnder99_BP_sigGO_subgraphs", useInfo = "all", pdfSW = TRUE)
printGraph(MF_GO_data, MF_GO_results, firstSigNodes = 5, 
           fn.prefix = "overPositiveUnder99_MF_sigGO_subgraphs", useInfo = "all", pdfSW = TRUE)
printGraph(CC_GO_data, CC_GO_results, firstSigNodes = 5, 
           fn.prefix = "overPositiveUnder99_CC_sigGO_subgraphs", useInfo = "all", pdfSW = TRUE)
