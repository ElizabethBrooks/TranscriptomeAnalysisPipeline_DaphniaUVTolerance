#!/usr/bin/env Rscript
#Usage: Rscript alignmentSummary_barPlotMedians_binnedCompareTarget.r alignmentSummaryFiles
#Usage Ex: Rscript alignmentSummary_barPlotMedians_binnedCompareTarget.r alignmentSummarized_hisat2_run1_formatted.csv alignmentSummarized_tophat2_run2_formatted.csv
...
counts <- data.frame(aStats.genotype, aStats$overall, aStats$concordant, aStats$target)
...
plotOverall <- ggplot(counts, aes(factor(aStats.genotype), aStats.overall, fill=aStats.target)) + 
...
plotConc <- ggplot(counts, aes(factor(aStats.genotype), aStats.concordant, fill=aStats.target)) + 
  geom_bar(stat="identity", position="dodge") + 
  ggtitle(plotTitle) +
  xlab("Sample Number") +
