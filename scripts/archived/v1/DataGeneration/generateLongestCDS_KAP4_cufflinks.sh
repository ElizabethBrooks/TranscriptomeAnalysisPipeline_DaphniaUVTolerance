#!/bin/bash
#Script to generate the longest CDS from the KAP4 gff and reference fasta
#Usage: qsub generateLongestCDS_KAP4_cufflinks.sh
...
tmpCDS="$outDir"/tmpKAP4_cds.fa
cat "$outCDS" | sed ':a;N;$!ba;s/\n/NEWLINE/g' | sed 's/NEWLINE>/\n>/g' > "$tmpCDS"

#Get list of CDS tags
tmpList="$outDir"/tmpKAP4_longest_cdsList.txt
...
outLongCDS="$outDir"/KAP4_longest_cds.fa
outLongCDSList="$outDir"/KAP4_longest_cds_list.txt
