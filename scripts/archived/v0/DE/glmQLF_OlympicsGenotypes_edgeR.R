...
design.test <- model.matrix(~treatment + treatment:genotype, data=targets)
fit.test <- glmQLFit(list, design.test)
# view the coefficient names 
colnames(fit.test)
# treatment (UV vs VIS)
qlf.treat.test <- glmQLFTest(fit.test, coef=2)
summary(decideTests(qlf.treat.test))
glm.treat.test <- glmTreat(fit.test, coef=2, lfc=log2(1.2))
summary(decideTests(glm.treat.test))

# What this method says is that the average gene expression of the treatment groups is more or less (+ or -) than the control group
