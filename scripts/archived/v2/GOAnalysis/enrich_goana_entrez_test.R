...
write.table(top_go, file='Sierra_top_go_terms_genome_3.0_entrezID.csv', sep=',', row.names=TRUE, col.names=TRUE)
...
kegg <- kegga(tested, geneid = rownames(tested))
top_kegg <- topKEGG(kegg)
write.table(top_kegg, file='Sierra_top_kegg_pathways_genome_3.0_entrezID.csv', sep=',', row.names=TRUE, col.names=TRUE)
