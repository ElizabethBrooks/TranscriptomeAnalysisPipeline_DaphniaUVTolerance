...
# interaction
inFile <- paste(deDir, "glmQLF_2WayANOVA_interaction_topTags.csv", sep="/")
geneCountsInter <- read.csv(file=inFile)
SETInterIn <- geneCountsInter[,1]
# treatment
inFile <- paste(deDir, "glmQLF_2WayANOVA_UVvsVIS_topTags.csv", sep="/")
geneCountsTreat <- read.csv(file=inFile)
SETTreatIn <- geneCountsTreat[,1]
# tolerance
inFile <- paste(deDir, "glmQLF_2WayANOVA_TvsN_topTags.csv", sep="/")
...
for(var in 1:length(colorList))
{
  ...
  SETInter <- which(names(datExprInter)[moduleColors==colorList[var]] %in% SETInterIn)
  SETTreat <- which(names(datExprInter)[moduleColors==colorList[var]] %in% SETTreatIn)
  SETTol <- which(names(datExprInter)[moduleColors==colorList[var]] %in% SETTolIn)
  ...
  colorSets[rowNum,2] <- length(SETInter)/length(names(datExprInter)[moduleColors==colorList[var]])*100
  colorSets[rowNum,3] <- "Interaction"
  #Add treatment data to second block
  rowNum = length(colorList)+var
  colorSets[rowNum,1] <- colorList[var]
  colorSets[rowNum,2] <- length(SETTreat)/length(names(datExprInter)[moduleColors==colorList[var]])*100
  colorSets[rowNum,3] <- "Treatment"
  #Add tolerance data to third block
  rowNum = length(colorList)*2+var
  colorSets[rowNum,1] <- colorList[var]
  colorSets[rowNum,2] <- length(SETTol)/length(names(datExprInter)[moduleColors==colorList[var]])*100
  colorSets[rowNum,3] <- "Tolerance"
  #Add intersection of all data to third block
  rowNum = length(colorList)*3+var
  colorSets[rowNum,1] <- colorList[var]
  colorSets[rowNum,2] <- length(SETIntersectAll)/length(names(datExprInter)[moduleColors==colorList[var]])*100
  ...
  colorSets[rowNum,1] <- colorList[var]
  colorSets[rowNum,2] <- length(SETInterTreat)/length(names(datExprInter)[moduleColors==colorList[var]])*100
  ...
  colorSets[rowNum,1] <- colorList[var]
  colorSets[rowNum,2] <- length(SETInterTol)/length(names(datExprInter)[moduleColors==colorList[var]])*100
  ...
  colorSets[rowNum,1] <- colorList[var]
  colorSets[rowNum,2] <- length(SETTreatTol)/length(names(datExprInter)[moduleColors==colorList[var]])*100
...
jpeg("stackedBarPlotInter_moduleEffectSubsets_signedNowick.jpg", width = 844, height = 596)
