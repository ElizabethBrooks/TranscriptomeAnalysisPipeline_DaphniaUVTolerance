...
par(mfrow=c(2,1))
#Set the plot titles
plotTitle <- "Comparison of Alignments to Genotypes"
...
outFile <- paste(normalizePath(dirname(args[1])), "plotMedianOverallPercentages.jpg", sep="/")
ggsave(outFile)
...
outFile <- paste(normalizePath(dirname(args[1])), "plotMedianConcordantPercentages.jpg", sep="/")
ggsave(outFile)
