	...
	if [ "$analysisMethod" == "hisat2" ]; then
		...
		if [ $? -eq 0 ]; then
			...
			for f3 in "$f1"/out/*; do
				...
				#using 8 threads
				samtools sort -@ 8 -n -o "$analysisFiles/${f3:(${#f1}+5):(${#f3}-${#analysisTag}-${#analysisFiles}+1)}".sorted.bam -T /tmp/"$analysisMethod"EdgeR_sorted_"${f3:(${#f1}+5):(${#f3}-${#analysisTag}-${#analysisFiles})}".sorted $f3
