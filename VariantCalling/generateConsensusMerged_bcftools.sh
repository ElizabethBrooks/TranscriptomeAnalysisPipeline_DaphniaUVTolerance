...
cat "$genomeFile" | bcftools consensus "$outFolder"/"$type"_calls.norm.flt-indels.bcf > "$path"/"$type"_consensus.fa
echo cat "$genomeFile" | bcftools consensus "$outFolder"/"$type"_calls.norm.flt-indels.bcf ">" "$path"/"$type"_consensus.fa > "$inputOutFile"