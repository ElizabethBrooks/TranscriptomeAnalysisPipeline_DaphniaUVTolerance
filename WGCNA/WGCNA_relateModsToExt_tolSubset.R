...
MEs0 = moduleEigengenes(datExprTol, moduleColors)$eigengenes
MEs = orderMEs(MEs0)
moduleTraitCor = cor(MEs, datTraitsTol, use = "p");
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);


#sizeGrWindow(10,6)
jpeg("moduleTraitRelationshipsTol.jpg", width = 960, height = 960)
...
geneModuleMembership = as.data.frame(cor(datExprTol, MEs, use = "p"));
MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership), nSamples));

names(geneModuleMembership) = paste("MM", modNames, sep="");
names(MMPvalue) = paste("p.MM", modNames, sep="");

geneTraitSignificance = as.data.frame(cor(datExprTol, tolerance, use = "p"));
...
write.csv(geneInfo, file = "geneInfoTol_tolerance.csv")
...
geneTraitSignificance = as.data.frame(cor(datExprTol, treatment, use = "p"));
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
...
write.csv(geneInfo, file = "geneInfoTol_treatment.csv")
