...
# The following is the number of treatment probes without annotation:
...
#Setup treatment inputs
geneIDList = probes
uniprotIDList = annot$uniprotID[probes2annot]
entrezIDList = annot$entrezID[probes2annot]


# Define numbers of treatment genes and samples
nGenes = ncol(datExprTreat);
nSamples = nrow(datExprTreat);
# Recalculate MEs with color labels
MEs0 = moduleEigengenes(datExprTreat, moduleColors)$eigengenes
MEs = orderMEs(MEs0)
moduleTraitCor = cor(MEs, datTraitsTreat, use = "p");
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);


#sizeGrWindow(10,6)
jpeg("moduleTraitRelationshipsTreat.jpg", width = 960, height = 960)
...
tolerance = as.data.frame(datTraitsTreat$tolerance);
names(tolerance) = "tolerance"
# names (colors) of the modules
modNames = substring(names(MEs), 3)

geneModuleMembership = as.data.frame(cor(datExprTreat, MEs, use = "p"));
MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership), nSamples));

names(geneModuleMembership) = paste("MM", modNames, sep="");
names(MMPvalue) = paste("p.MM", modNames, sep="");

geneTraitSignificance = as.data.frame(cor(datExprTreat, tolerance, use = "p"));
...
write.csv(geneInfo, file = "geneInfoTreat_tolerance.csv")


# Define variable treatment containing the treatment column of datTrait
treatment = as.data.frame(datTraitsTreat$treatment);
...
geneTraitSignificance = as.data.frame(cor(datExprTreat, treatment, use = "p"));
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
...
write.csv(geneInfo, file = "geneInfoTreat_treatment.csv")
