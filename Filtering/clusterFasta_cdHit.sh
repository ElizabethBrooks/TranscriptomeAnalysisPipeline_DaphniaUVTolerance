...
#Usage Ex: bash clusterFasta_cdHit.sh sequence 0.98 sortedCoordinate_samtoolsHisat2_run1Y05
#Usage Ex: bash clusterFasta_cdHit.sh sequence 0.98 sortedCoordinate_samtoolsHisat2_run1Y05
#Default usage Ex: bash clusterFasta_cdHit.sh sequence 0.98 assemblyTrinity_all
...
if [[ "$3" == sorted* ]]; then
	...
	outputFolder="$outputsPath/$3""_assemblyGenomeTrinity_multiFasta"
	#Set merged fasta file name
	multiFastaFile="$outputFolder/assemblyGenomeTrinity_multiFasta.fasta"
	summaryFile="$outputFolder/$3""_assemblyGenomeTrinity_multiFasta_summary.txt"
	...
	for i in "$@"; do
		...
		if [[ $counter -ge 4 ]]; then
			...
			fastaFile="$inputsPath/$3$i""_assemblyGenomeTrinity/Trinity.fasta "
			"$softsPath"/cd-hit-est -o "$outputFolder"/cdhit -c $2 -i "$fastaFile" -p 1 -d 0 -b 3 -T 10
		fi
		counter=$(($counter+1))
	done
elif [[ "$3" == trimmed* ]]; then
	...
	inputsPath=$(grep "assembling:" ../InputData/outputPaths.txt | tr -d " " | sed "s/assembling://g")
	#Create output directory
	outputFolder="$outputsPath/$3""_assemblyTrinity_multiFasta"
	#Set merged fasta file name
	multiFastaFile="$outputFolder/assemblyTrinity_multiFasta.fasta"
	summaryFile="$outputFolder/$3""_assemblyTrinity_multiFasta_summary.txt"
	#Retrieve selected fasta files
	#Loop through all input genotypes and merge fasta files
	for i in "$@"; do
		#Skip first three arguments
		if [[ $counter -ge 5 ]]; then
			#Set current fasta file path
			fastaFile="$inputsPath/$3$i""_assemblyTrinity/Trinity.fasta "
			"$softsPath"/cd-hit-est -o "$outputFolder"/cdhit -c $2 -i "$fastaFile" -p 1 -d 0 -b 3 -T 10
		fi
		counter=$(($counter+1))
	done
	...
	#Create output directory
	outputFolder="$outputsPath/$3_multiFasta"
	#Set merged fasta file name
	multiFastaFile="$outputFolder/$3""_multiFasta.fasta"
	summaryFile="$outputFolder/$3""_multiFasta_summary.txt"
	#Retrieve selected fasta files
	#Loop through all input genotypes and merge fasta files
	for i in "$@"; do
		#Set current fasta file path
		if [[ $counter -ge 4 ]]; then
			"$softsPath"/cd-hit-est -o "$outputFolder"/cdhit -c $2 -i "$i" -p 1 -d 0 -b 3 -T 10
		fi
		counter=$(($counter+1))
	done
fi
