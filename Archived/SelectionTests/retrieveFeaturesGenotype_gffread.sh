...
#$ -m abe
#$ -r n
#$ -N retrieveFeaturesGenotype_jobOutput

# script to retrieve features from a referene fasta using a gff
# usage: bash retrieveFeaturesGenotype_gffread.sh
...
# E05
gffread -C -y $outFolder"/"$type"_consensus_E05_longest.pep.fa" -x $outFolder"/"$type"_consensus_E05_longest.cds.fa" -g $inputsPath"/"$type"_consensus_E05.fa" $consensusFeatures
# R2
gffread -C -y $outFolder"/"$type"_consensus_R2_longest.pep.fa" -x $outFolder"/"$type"_consensus_R2_longest.cds.fa" -g $inputsPath"/"$type"_consensus_R2.fa" $consensusFeatures
# Y05
gffread -C -y $outFolder"/"$type"_consensus_Y05_longest.pep.fa" -x $outFolder"/"$type"_consensus_Y05_longest.cds.fa" -g $inputsPath"/"$type"_consensus_Y05.fa" $consensusFeatures
# Y023
gffread -C -y $outFolder"/"$type"_consensus_Y023_longest.pep.fa" -x $outFolder"/"$type"_consensus_Y023_longest.cds.fa" -g $inputsPath"/"$type"_consensus_Y023.fa" $consensusFeatures
